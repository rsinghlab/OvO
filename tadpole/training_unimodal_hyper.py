...
def train_model(modality_name, dataloaders, criterion, config, path):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            mauc = MAUC(all_probabilities, num_classes)
            bca = calcBCA(all_predictions, all_labels, num_classes)

            print('{} Loss: {:.4f} Acc: {:.4f} F1: {:.4f} MAUC: {:.4f} BCA: {:.4f}'.format(
                phase, epoch_loss, epoch_acc, epoch_f1, mauc, bca
            ))

            if phase == 'val':
                wandb.log({"val_loss": epoch_loss, "val_acc": epoch_acc,  "val_f1": epoch_f1, "val_mauc": mauc, "val_bca": bca}) 
            ...
            if phase == 'train':
                wandb.log({"train_loss": epoch_loss, "train_acc": epoch_acc,"train_f1": epoch_f1, "train_mauc": mauc, "train_bca": bca, "epoch": epoch}) #this logs to weights and biases
...
def main():
    ...
    train_inputs = torch.load((f'{data_path}/{modality_name}_train_dataset.pt'))
    val_inputs = torch.load((f'{data_path}/{modality_name}_val_dataset.pt'))
    ...
    # Initialize a new wandb run
        with wandb.init(config=config):
            ...
            path = save_model_path + '/models/model_' + str(config.learning_rate) + "_" + str(config.random_seed) + "_" + str(config.batch_size) + "_" + str(config.epochs) + "_" + modality_name 
